...
window.onload = function () {
  ...
  for (let i = 1; i <= rows * columns; i++) {
     ...
     * }
     */
    blanks.push({ index: i, placedImage: -1 }); // puts objects named 1-16 into the index, and a placeholder number (-1) in placedImage
  }
  ...
  for (let i = 1; i <= rows * columns; i++) {
    pieces.push(i.toString()); // puts puzzle images (named 1-16) into the array
  }
  ...
  for (let i = 0; i < pieces.length; i++) {
    let tile = document.createElement("img");
    tile.src = "./puzpics/debussy/" + pieces[i] + ".jpg";
...
function dragEnd() {
  ...
  if (currentTile.src.includes("debussy")) {
...
function validate() {
  ...
  if (isSolved) {
    window.location.href = "puzzleMedSolved.html";
  }
}
